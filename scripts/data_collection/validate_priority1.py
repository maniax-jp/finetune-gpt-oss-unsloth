#!/usr/bin/env python3
"""
Validate Priority 1 Collection Data
Priority 1カテゴリー収集データの検証
"""
...
def main():
    """メイン実行関数"""
    logger.info("=== Validating Priority 1 Collection Data ===")

    # Priority 1データを読み込み
    priority1_file = "data/processed/priority1_collection.json"
    with open(priority1_file, 'r', encoding='utf-8') as f:
        priority1_data = json.load(f)

    logger.info(f"Loaded {len(priority1_data)} QA pairs from {priority1_file}")

    # バリデーター初期化
    validator = QAValidator()

    # 検証実行
    results = validator.validate_dataset(priority1_data)
    ...
    # レポートをファイルに保存
    report_file = "data/metadata/priority1_validation_report.txt"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write(report)
    logger.info(f"\nReport saved to {report_file}")

    # 重複チェック
    duplicates = validator.check_duplicates(priority1_data)
    if duplicates:
...
if __name__ == "__main__":
    ...
    if result['valid']:
        print("\n" + "=" * 80)
        print("✅ PRIORITY 1 COLLECTION PASSED ALL QUALITY CHECKS")
        print("=" * 80)
    else:
        print("\n" + "=" * 80)
        print("⚠️ PRIORITY 1 COLLECTION HAS QUALITY ISSUES - REVIEW REQUIRED")
        print("=" * 80)
