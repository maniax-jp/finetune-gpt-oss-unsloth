#!/usr/bin/env python3
"""
第6次ファインチューニングスクリプト - 固有名詞認識対応版

Phase 10-2-A: 固有名詞認識問題の解決

改善点:
1. 混合データセットの使用
   - 推論データ: 50サンプル（固有名詞の日英対応を明示）
   - 非推論データ: 455サンプル（既存データ）
   - 推論データ比率: 9.9%

2. 固有名詞認識データの特徴
   - Analysis channelに「高市早苗 = Sanae Takaichi」を明示
   - 重要な固有名詞の日英対応を含む
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/training_sixth_finetune.log'),
...
# データセットパス（第6次: 混合データセット）
MIXED_DATASET_PATH = "dataset/takaichi_sanae_mixed_v6.jsonl"

# 出力ディレクトリ
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR = f"outputs/gpt-oss-20b-takaichi-v6-proper-nouns-{timestamp}"
...
logger.info("=" * 60)
logger.info(f"Dataset path: {MIXED_DATASET_PATH}")
logger.info(f"  推論データ: 50サンプル（固有名詞認識特化）")
logger.info(f"  非推論データ: 455サンプル（既存データ）")

dataset = load_dataset("json", data_files=MIXED_DATASET_PATH, split="train")
