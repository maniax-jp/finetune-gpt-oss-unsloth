...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/training_second_finetune_optimized.log'),
...
# トレーニング設定（最適化版）
BATCH_SIZE = 4  # 2→4（GPU並列度2倍）
GRADIENT_ACCUMULATION_STEPS = 4  # 実効バッチサイズ=16（通常版の8から2倍）
...
OUTPUT_DIR = f"outputs/gpt-oss-20b-takaichi-v2-optimized-{timestamp}"
...
logger.info("=" * 80)
logger.info("第2次ファインチューニング開始（最適化版）")
logger.info("=" * 80)
logger.info(f"\n【通常版からの最適化】")
logger.info(f"  バッチサイズ: 2 → {BATCH_SIZE} (2倍)")
logger.info(f"  実効バッチサイズ: 8 → {BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS} (2倍)")
logger.info(f"  予想VRAM使用量: 11.74GB → 約20-23GB")
logger.info(f"  予想高速化: 約50%削減\n")
logger.info(f"\n【第1次からの改善点】")
...
logger.info("\n" + "=" * 80)
logger.info("🎉 第2次ファインチューニング完了（最適化版）")
logger.info("=" * 80)
